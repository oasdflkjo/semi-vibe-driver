...
static bool process_command(const char *command, char *response)
{
    ...
    if (!protocol_parse_message(command, &msg))
    {
        // Invalid format
        protocol_create_error(ERROR_FORBIDDEN, &msg);
        protocol_format_message(&msg, response);
        return true;
    }
    ...
    {
        protocol_create_error(ERROR_INVALID, &msg);
        protocol_format_message(&msg, response);
        return true;
    }
